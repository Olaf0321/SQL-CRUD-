...
export default function DashboardPage() {
  ...
  const [selectListColumn, setSelectListColumn] = useState<{ [key: string]: any[] }>({});
  const [fkcolumnName, setFkColumnName] = useState<string>("");
  const [fkColumnToParent, setFkColumnToParent] = useState<{ [columnName: string]: string }>({});
  const [fkParentRecords, setFkParentRecords] = useState<{ [parentTable: string]: any[] }>({});
  const [fkDropdownOpen, setFkDropdownOpen] = useState<{ [columnName: string]: boolean }>({});
  const [fkSearchText, setFkSearchText] = useState<{ [columnName: string]: string }>({});
  ...
  // Derive FK columns for the selected table by checking all tables as potential parents via backend lookup
  useEffect(() => {
    const deriveForeignKeys = async () => {
      try {
        if (!selectedDatabase || !selectedTableName) return;
        // Consider every table except the selected one as a potential parent
        const parents: string[] = (tables || []).filter(t => t && t !== selectedTableName);
        const newMap: { [column: string]: string } = {};
        for (const parentName of parents) {
          const res = await fetch(`${SERVER_URL}fkcolumnName/${encodeURIComponent(parentName)}/${encodeURIComponent(selectedTableName)}`, {
            headers: { ...buildDbHeaders() }
          });
          const data = await res.json();
          const fkCol = data;
          if (fkCol) {
            newMap[fkCol] = parentName;
          }
        }

        // Heuristic fallback: columns ending with 'ID' that match a table name
        const tableSetLower = new Set((tables || []).map(t => String(t).toLowerCase()));
        tableColumnsWithoutID.forEach(col => {
          if (newMap[col]) return;
          const colLower = String(col).toLowerCase();
          if (colLower.endsWith('id')) {
            const base = colLower.replace(/id$/, '');
            // Try exact match first; else try plural/singular naive forms
            if (tableSetLower.has(base)) {
              const parent = (tables || []).find(t => String(t).toLowerCase() === base) as string;
              if (parent) newMap[col] = parent;
            } else if (tableSetLower.has(base + 's')) {
              const parent = (tables || []).find(t => String(t).toLowerCase() === (base + 's')) as string;
              if (parent) newMap[col] = parent;
            } else if (base.endsWith('s') && tableSetLower.has(base.slice(0, -1))) {
              const parent = (tables || []).find(t => String(t).toLowerCase() === base.slice(0, -1)) as string;
              if (parent) newMap[col] = parent;
            }
          }
        });
        setFkColumnToParent(newMap);

        // Fetch parent records for each FK parent table
        const parentRecs: { [parent: string]: any[] } = {};
        for (const parentName of Object.values(newMap)) {
          const dbName = selectedDatabase['データベース名'];
          const res = await fetch(`${SERVER_URL}items/${encodeURIComponent(dbName)}/${encodeURIComponent(parentName)}`, {
            headers: { ...buildDbHeaders() }
          });
          const json = await res.json();
          parentRecs[parentName] = json.records || [];
        }
        setFkParentRecords(parentRecs);
      } catch (e) {
        console.error('FK derivation failed:', e);
      }
    };
    deriveForeignKeys();
  }, [tables, selectedTableName, selectedDatabase]);
      ...
      <div className="w-3/4 p-6 overflow-y-auto">
        ...
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogContent>
            ...
            {tableColumnsWithoutID.map((col, id) => (
              listCheck(col) == false ? (
                (fkColumnToParent[col] || fkcolumnName === col) ? (
                  <div key={col} className={`mb-4`}>
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={(e: any) => setFormData({ ...formData, [col]: e.target.value })}
                        placeholder={''}
                        className={`mt-1 p-2 border border-gray-300 rounded-md w-full pr-20`}
                      />
                      <button
                        type="button"
                        className="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1 text-sm bg-blue-500 hover:bg-blue-600 text-white rounded"
                        onClick={() => setFkDropdownOpen(prev => ({ ...prev, [col]: !prev[col] }))}
                      >
                        検索
                      </button>
                      {fkDropdownOpen[col] && (
                        <div className="absolute z-50 mt-2 w-full rounded-md border border-gray-200 bg-white shadow-lg max-h-64 overflow-auto">
                          <div className="p-2 border-b">
                            <input
                              type="text"
                              value={fkSearchText[col] || ''}
                              onChange={(e) => setFkSearchText(prev => ({ ...prev, [col]: e.target.value }))}
                              placeholder="検索..."
                          ...
                          <ul className="max-h-52 overflow-y-auto">
                            {(() => {
                              const parent = fkColumnToParent[col];
                              const all = fkParentRecords[parent] || [];
                              const q = (fkSearchText[col] || '').toLowerCase();
                              const filtered = q
                                ? all.filter((r: any) => Object.values(r).some((v: any) => String(v ?? '').toLowerCase().includes(q)))
                                : all;
                              return filtered.map((r: any, idx: number) => (
                                <li
                                  ...
                                  onClick={() => {
                                    setFormData(prev => ({ ...prev, [col]: String(r.ID) }));
                                    setFkDropdownOpen(prev => ({ ...prev, [col]: false }));
                                  }}
                                >
                                  <div className="text-sm font-medium">ID: {String(r.ID ?? '')}</div>
                                  <div className="text-xs text-gray-500 truncate">{Object.entries(r).filter(([k]) => k !== 'ID' && k !== 'id').slice(0, 3).map(([k, v]) => `${k}:${v}`).join(' / ')}</div>
                                </li>
                              ));
                            })()}
                          </ul>
            ...
            <div className="mt-4 flex justify-end space-x-2">
              ...
              <Button onClick={()=>{
                ...
                // formData is empty, return
                if (Object.keys(formData).length === 0) {
                  alert('正確に入力してください。');
                  return;
                }

                // if formdata lenght is not same as tableColumnsWithoutID, return
                if (Object.keys(formData).length !== tableColumnsWithoutID.length) {
                  alert('正確に入力してください。');
                  return;
                }
                
                // input value is empty, return
                Object.entries(formData).forEach(([key, value]) => {
